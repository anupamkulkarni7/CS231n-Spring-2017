...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    loss = -1*np.sum(np.log(a2[np.arange(N),y]))/N
    ...
    gradW1 = np.dot(X.T,gradz1)/N
    
    grads['W1'] = gradW1 + 2.0*reg*W1
    grads['b1'] = gradb1
    grads['W2'] = gradW2 + 2.0*reg*W2
    grads['b2'] = gradb2
  ...
  def train(self, X, y, X_val, y_val,
    ...
    for it in xrange(num_iters):
      ...
      #########################################################################
      indx = np.random.choice(N,batch_size)
      X_batch = X[indx]
      y_batch = y[indx]
