...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        xhat = (x - ex)/np.sqrt(varx + eps)
        out = xhat*gamma + beta
        
        cache = (xhat,ex,varx,gamma)
...
def batchnorm_backward(dout, cache):
    ...
    dxhat = dout*gamma
    #dx = dxhat/np.sqrt(varx)
    
    dx = (1. / N) * (1./np.sqrt(varx)) * (N*dxhat - np.sum(dxhat, axis=0) 
